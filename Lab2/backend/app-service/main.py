...
RECOMMENDER_URL = os.getenv("RECOMMENDER_URL", "http://recommender-svc:8002")
...
async def recommend_forward(book_id: int):
    async with httpx.AsyncClient() as client:
        r = await client.get(f"{RECOMMENDER_URL}/recommend/{book_id}")
        return r.json()
